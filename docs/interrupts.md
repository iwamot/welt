...
- **Multiple questions**: one stop can carry several. Welt renders them all in one message and resumes only after every one is answered — there is no partial resume, since most of the frameworks the adapters cover resume a run as a whole rather than a question at a time. A resumed run may stop again; the round trip just repeats. One message holds 50 blocks, and a question takes its body plus one block per widget, so a stop fits up to 25 questions, or 16 when each carries both buttons and a text field; a stop needing more is refused as a whole and the thread shows the reply failure notice.
...
- **Expiry**: Welt sets no deadline — an answer always just attempts the resume, and when the run can no longer continue, a notice appears under the questions. How long a run stays resumable is between the agent and its runtime. On AgentCore Runtime, the paused run lives in the session's compute, and the runtime's [lifecycle configuration](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-lifecycle-settings.html) stops that compute once the session has been idle for `idleRuntimeSessionTimeout` (default 15 minutes) or once the compute has lived for `maxLifetime` (default 8 hours). The session itself survives — the next invocation gets fresh compute and the thread carries on — but the run that was waiting for the answer went with the old one, so a question left open past the idle timeout cannot be resumed unless the agent keeps its paused state outside the compute. Deployers who need more time for approvals raise `idleRuntimeSessionTimeout` on the runtime.
